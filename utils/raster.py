...
def raster2uint8(im, norm=True, stretch=False):
    """
    ...
    # 2% linear stretch
    def _two_percent_linear(image, max_out=1.0, min_out=0.0):
        def _gray_process(gray, maxout=max_out, minout=min_out):
            ...
            processed_gray = (
                (truncated_gray - low_value) / (high_value - low_value + EPS)
            ) * (maxout - minout)
...
class Raster:
    ...
    def setBands(self, band_list: Union[List[int], Tuple[int], None]) -> None:
        ...
        if band_list is not None:
            ...
            if max(band_list) > self.bands or min(band_list) < 1:
                raise ValueError(
                    "The range of band_list must within [1, {0}].".format(
                        str(self.bands)
                    )
                )
    ...
    def _getArray(
        self, window: Union[None, List[int], Tuple[int, int, int, int]] = None
    ) -> np.ndarray:
